...
> 🚀 **Cloudflare Workers + KV 边缘计算架构** | **纯数学 BigInt RSA2 (RSASSA-PKCS1-v1_5 SHA-256) 签名与验签** | **零服务器自建极简收银台** | **支付宝异步 Webhook 秒级自动发卡** | **RSA-PSS 硬件指纹强绑定** | **云端可视化财务与授权控制台**
...
- 📱 **响应式收银台 (`/pay`)**：极简现代科技暗黑风格，自适应 PC 扫码与手机端唤起支付宝 App。
- 🔑 **硬件指纹绑定自动发卡 (`/api/pay/alipay-callback`)**：支付成功后自动根据机器码签发不可伪造的专属 RSA-PSS 商业授权码。
- 📊 **云端财务与授权控制台 (`/admin`)**：实时订单流水、销售总额统计、在线远程封禁/解封/续期。
...
sequenceDiagram
    ...
    User->>Cashier: 打开收银台，输入机器码 (MID)，选择套餐
    Cashier->>Worker: POST /api/pay/create-order
    ...
    Worker->>Worker: 验签支付宝公钥，签发专属 RSA-PSS 授权码
