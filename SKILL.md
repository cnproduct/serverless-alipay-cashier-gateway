...
1. **按店铺计费与单店专属授权 (¥600 RMB / 店铺/年 · 365天有效)**：
   - 收费模式：每家 Wildberries 店铺收费 600 元人民币（自支付当日起 365 天有效，一店一码 1:1 独立互斥锁定）；
   - 支付宝扫码支付 600 元，系统秒级自动签发 1 个专属商业授权码并自动激活当前会话窗口；
   - 授权码与 Wildberries 店铺 ID / OpenAPI Token 永久锁定，彻底杜绝轮播白嫖与商品串店错乱隐患。
...
4. **极简现代暗黑风自适应收银台 (`/pay?cid=...`)**：
   - 适配 PC 浏览器与移动端手机屏幕；
   - 自动提取并带入聊天窗口专属 ID（`Conversation ID`）与 Wildberries 店铺名称，默认单店 ¥600.00，一键唤起支付宝扫码或手机 App 支付。
   - 客户端聊天界面使用标准行内 CSS 限制的 100×100 px（约 3cm × 3cm）紧凑二维码，杜绝界面撑大变形。

5. **异步 Webhook 自动发卡与免手动激活 (`/api/pay/alipay-callback`)**：
   - 严格验签支付宝公钥；
   - 验签成功后，自动根据会话 ID 与店铺名称生成不可伪造的专属 RSA-PSS 2048-bit 商业授权码（`LIC-RSA-...`）并持久化至 Cloudflare KV，同时秒级通知客户端窗口自动激活。
